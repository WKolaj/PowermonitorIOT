...
let FactorElementVariableCreateSchema = Joi.object().keys({
  type: Joi.string()
    .valid("factorElement")
    .required(),
  name: Joi.string()
...
let FactorElementVariableEditSchema = Joi.object().keys({
  name: Joi.string()
    .min(3)
    .max(100),
  sampleTime: Joi.number()
...
let validateCreate = function(req) {
  ...
  return new Promise(async (resolve, reject) => {
    Joi.validate(
      req.body,
      FactorElementVariableCreateSchema,
      async (err, value) => {
        if (err) {
...
let validateEdit = function(req) {
  return new Promise(async (resolve, reject) => {
    Joi.validate(
      req.body,
      FactorElementVariableEditSchema,
      async (err, value) => {
        if (err) {
          return resolve(err.details[0].message);
        } else {
