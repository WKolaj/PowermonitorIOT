...
let MBSwappedInt32VariableCreateSchema = Joi.object().keys({
  type: Joi.string()
    .valid("mbSwappedInt32")
    .required(),
  name: Joi.string()
    .min(3)
    .max(100)
...
let MBSwappedInt32VariableEditSchema = Joi.object().keys({
  name: Joi.string()
    .min(3)
    .max(100),
  sampleTime: Joi.number()
...
let validateCreate = function(req) {
  ...
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, MBSwappedInt32VariableCreateSchema, (err, value) => {
      if (err) {
...
let validateEdit = function(req) {
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, MBSwappedInt32VariableEditSchema, (err, value) => {
      if (err) {
        return resolve(err.details[0].message);
      } else {
        return resolve();
