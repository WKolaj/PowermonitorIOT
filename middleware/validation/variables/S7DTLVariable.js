...
let S7DTLVariableCreateSchema = Joi.object().keys({
  type: Joi.string()
    .valid("s7DTL")
    .required(),
  name: Joi.string()
...
let S7DTLVariableEditSchema = Joi.object().keys({
  name: Joi.string()
    .min(3)
    .max(100),
  sampleTime: Joi.number()
...
let validateCreate = function(req) {
  ...
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, S7DTLVariableCreateSchema, (err, value) => {
      if (err) {
...
let validateEdit = function(req) {
  return new Promise(async (resolve, reject) => {
    ...
    Joi.validate(req.body, S7DTLVariableEditSchema, (err, value) => {
      if (err) {
        return resolve(err.details[0].message);
      } else {
        return resolve();
