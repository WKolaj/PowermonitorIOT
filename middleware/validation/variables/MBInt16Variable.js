...
let MBInt16VariableCreateSchema = Joi.object().keys({
  type: Joi.string()
    .valid("int16")
    .required(),
  name: Joi.string()
...
let MBInt16VariableEditSchema = Joi.object().keys({
  name: Joi.string()
    .min(3)
    .max(100),
  timeSample: Joi.number()
...
let validateCreate = function(req) {
  ...
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, MBInt16VariableCreateSchema, (err, value) => {
      if (err) {
...
let validateEdit = function(req) {
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, MBInt16VariableEditSchema, (err, value) => {
      if (err) {
        return resolve(err.details[0].message);
      } else {
        return resolve();
