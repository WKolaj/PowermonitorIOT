...
let MBUInt32VariableCreateSchema = Joi.object().keys({
  type: Joi.string()
    .valid("uInt32")
    .required(),
  name: Joi.string()
...
let MBUInt32VariableEditSchema = Joi.object().keys({
  name: Joi.string()
    .min(3)
    .max(100),
  timeSample: Joi.number()
...
let validateCreate = function(req) {
  ...
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, MBUInt32VariableCreateSchema, (err, value) => {
      if (err) {
...
let validateEdit = function(req) {
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, MBUInt32VariableEditSchema, (err, value) => {
      if (err) {
        return resolve(err.details[0].message);
      } else {
        return resolve();
