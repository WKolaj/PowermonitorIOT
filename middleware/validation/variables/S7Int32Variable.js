...
let S7Int32VariableCreateSchema = Joi.object().keys({
  type: Joi.string()
    .valid("s7Int32")
    .required(),
  name: Joi.string()
...
let S7Int32VariableEditSchema = Joi.object().keys({
  name: Joi.string()
    .min(3)
    .max(100),
  sampleTime: Joi.number()
...
let validateCreate = function(req) {
  ...
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, S7Int32VariableCreateSchema, (err, value) => {
      if (err) {
...
let validateEdit = function(req) {
  return new Promise(async (resolve, reject) => {
    ...
    Joi.validate(req.body, S7Int32VariableEditSchema, (err, value) => {
      if (err) {
        return resolve(err.details[0].message);
      } else {
        return resolve();
