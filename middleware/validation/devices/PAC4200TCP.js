...
let PAC4200TCPCreateSchema = Joi.object().keys({
  type: Joi.string()
    .valid("PAC4200TCP")
    .required(),
  name: Joi.string()
...
let PAC4200TCPEditSchema = Joi.object().keys({
  name: Joi.string()
    .min(3)
    .max(100),
  timeout: Joi.number()
...
let validateCreate = function(req) {
  ...
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, PAC4200TCPCreateSchema, (err, value) => {
      if (err) {
...
let validateEdit = function(req) {
  return new Promise(async (resolve, reject) => {
    Joi.validate(req.body, PAC4200TCPEditSchema, (err, value) => {
      if (err) {
        return resolve(err.details[0].message);
      } else {
        return resolve();
