...
const S7DTLVariable = require("../../../classes/variable/S7/S7DTLVariable");
...
describe("S7DTLVariable", () => {
  ...
  describe("init", () => {
    ...
    beforeEach(async () => {
      ...
      //1582791689852 -> 2020-02-27T08:21:29.852Z
    });

    let exec = async () => {
      device = new S7Device();
      await device.init(devicePayload);
      variable = new S7DTLVariable(device);
    ...
    it("should set value of variable to  if it is given in payload", async () => {
      variablePayload.value = 1582792818201;
      await exec();

      expect(variable.Value).toEqual(1582792818201);
    });

    it("should set value of variable to 1899-12-30T22:36:00.000Z, -2209080240000 (0 - year, 0-month, 0-day)if it is not defined in payload", async () => {
      variablePayload.value = undefined;
      await exec();

      expect(variable.Value).toEqual(-2209080240000);
    });
    ...
    it("should set length to 12 if it is not defined in payload", async () => {
    ...
    it("should set length to 12 even if it is defined in payload as a different value", async () => {
  ...
  describe("editWithPayload", () => {
    ...
    it("should set new value if it is defined in payload", async () => {
      editPayload = { value: 1582792925470 };
