...
const S7UInt32Variable = require("../../../classes/variable/S7/S7UInt32Variable");
...
describe("S7UInt32Variable", () => {
  ...
  describe("init", () => {
    ...
    let exec = async () => {
      ...
      variable = new S7UInt32Variable(device);
  ...
  describe("_convertDataToValue", () => {
    ...
    it("should return the same value - if value should be less than 0 if int", async () => {
      data = [255, 2, 3, 4];

      let result = await exec();

      expect(result).toEqual(4278321924);
    });
  ...
  describe("_convertValueToData", () => {
    ...
    it("should return proper data - if value is smaller than 0 as int", async () => {
      value = 4278321924;
