...
describe("events route", () => {
  ...
  describe("GET /:deviceId/:elementId", () => {
    ...
    it("should return code 200 and valid payload of last Event", async () => {
      let result = await exec();

      expect(result.status).toEqual(200);

      let expectedPayload = {
        [mbSwappedInt32Variable.Value]:
          eventLog.Descriptions[mbSwappedUInt32Variable.Value]
      };

      expect(result.body).toEqual(expectedPayload);
    });

    it("should return code 200 and valid payload of last Event - if event value is not presented in descriptions", async () => {
      mbSwappedUInt32Variable.Value = 1234;
      ...
      let expectedPayload = {
        [mbSwappedInt32Variable.Value]: mbSwappedUInt32Variable.Value
      };

      expect(result.body).toEqual(expectedPayload);
    });

    it("should return code 200 and valid payload of last Event - if there is no event", async () => {
