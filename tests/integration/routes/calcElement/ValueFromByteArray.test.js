...
describe("valueFromByteArray route", () => {
  ...
  let init = async () => {
    ...
    let mbByteArrayResult = await request(server)
      .post(`/api/variables/${mbDevice.Id}`)
      .set(tokenHeader, adminToken)
      .send(mbByteArrayBody);
  ...
  describe("CREATE /:deviceId", () => {
    ...
    it("should create new calcElement based on given payload", async () => {
      ...
      expect(newElement.Payload).toEqual({
        ...calcElementBody,
        id: newElement.Id
      });
    });
    ...
    it("should return code 400 and do not create element if variable in payload has different type than byteArray", async () => {
      calcElementBody.variableId = mbFloatVariable.Id;
    ...
    it("should return code 200 and create element with default bitNumber if bitNumber is undefined", async () => {
      calcElementBody.bitNumber = undefined;

      let result = await exec();

      expect(result.status).toEqual(200);
      expect(result.body.bitNumber).toEqual(0);
    });

    it("should return code 200 and create element with default byteNumber if byteNumber is undefined", async () => {
      calcElementBody.byteNumber = undefined;

      let result = await exec();

      expect(result.status).toEqual(200);
      expect(result.body.byteNumber).toEqual(0);
    });

    it("should return code 200 and create element with default length if length is undefined", async () => {
      calcElementBody.length = undefined;
